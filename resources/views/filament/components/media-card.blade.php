...
@else
    @php
        ...
        $base64Data = \App\Support\FilamentUI::getBase64DataUrl($media);
    @endphp
    ...
    <div x-data="{ 
        ...
        <div style="display: flex; align-items: center; border: 1px solid #e5e7eb; border-radius: 12px; padding: 16px; background-color: #f9fafb; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05); gap: 16px;">
            ...
            <div style="flex-grow: 1; text-align: left; min-width: 0;">
                <h4 style="font-size: 14px; font-weight: 600; color: #111827; margin: 0 0 2px 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $media->original_name }}">{{ $media->original_name }}</h4>
                <p style="font-size: 12px; color: #6b7280; margin: 0;">{{ strtoupper($media->extension) }} • {{ $sizeLabel }}</p>
            </div>
            <button @click="open = true; initBlob('{{ $base64Data }}'); $event.preventDefault();" style="cursor: pointer; display: inline-flex; align-items: center; justify-content: center; padding: 8px 16px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 12px; font-weight: 600; color: #2563eb; background-color: #ffffff; text-decoration: none; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05); transition: background-color 0.2s; flex-shrink: 0;" onmouseover="this.style.backgroundColor='#f3f4f6'" onmouseout="this.style.backgroundColor='#ffffff'">
